def some?(arr, &blk)
    arr.each { |ele| return true  if blk.call(ele) }
...
def filter_out(arr, &blk)
    answer = []
    arr.each {|ele| answer << ele if !blk.call(ele)}
    answer
end
...
def at_least?(arr, n, &blk)
    count = 0
    arr.each {|ele| count += 1 if blk.call(ele)}#.count >= n
    return count >= n
end
...
def every?(arr, &blk)
    arr.each { |ele| return false if !blk.call(ele)}
    return true
end
...
def xnor_select(arr, proc1, proc2)
    ...
    # arr.reject { |ele| !proc1.call(ele) && proc2.call(ele) || proc1.call(ele) && !proc2.call(ele)}
end
...
def proctition(arr, &blk)
    ...
    # arr.each { |ele| blk.call(ele) ? accept << ele : reject << ele }
    # [ *trues, *falses ]
    # accept + reject
    answer = arr.partition { |ele| blk.call(ele)}
    answer[0] + answer[1]
end
...
def selected_map!(arr, proc1, proc2)
    arr.each_with_index { |ele, i| arr[i] = proc2.call(ele) if proc1.call(ele) }
    return nil
end
...
def proctition_platinum(arr, *prc)
    ...
    prc.each_with_index { |proc, i| hash[i+1] = [] }

    arr.each do |ele|
        prc.each_with_index do |proc, i|
            if proc.call(ele)
                hash[i+1] << ele 
                break
            end
